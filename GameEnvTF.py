...
class Game2048PyEnv(py_environment.PyEnvironment):
  ...
  def _step(self, action):
    ...
    illegalmove = Move.NOTHING
    if (action == Move.UP or action == Move.RIGHT):
        optimal = findOptimalMove(self._state.toIntArray())
        if (optimal != action):
            action = optimal
            illegalmove = action
    ...
    if (illegalmove in legal_moves):
      legal_moves.remove(illegalmove)
